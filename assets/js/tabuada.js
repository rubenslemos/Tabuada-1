...
criarTabuada = () => {
  ...
  const adicionarMenus = (fimIntervalo) => {
    ...
    menus.forEach((menu) => {
      ...
      if (!elemento) {
        console.error(
          `Elemento submenu não encontrado para ${menu.id}. Verifique o HTML e o id.`
        )
        return
      }
      ...
      for (let inicio = 1; inicio <= fimIntervalo; inicio += 10) {
        ...
        for (let i = inicio; i <= fim; i++) {
          ...
          link.setAttribute(
            'value',
            `${menu.classe}${i.toString().padStart(2, '0')}`
          )
  ...
  navList.addEventListener('click', function (event) {
    ...
    // Pegamos o <a> mais próximo (cobre clique no texto e no <i> dentro do <a>)
    const linkElement = target.closest('a')
    if (linkElement) {
      // Só intercepta os links dos menus operacionais. Os demais (Desempenho, Permissões, Sair, etc.)
      // devem seguir o href normalmente.
      const isOpLink =
        linkElement.classList.contains('soma') ||
        ...
        linkElement.classList.contains('intervalo-link')

      if (!isOpLink) {
        return // deixa o navegador navegar para o href
      }
      ...
      if (
        ...
        // Verifica se o clique foi em um link de intervalo (submenu)
          ...
          linkElement.classList.contains('intervalo-link') &&
          linkElement.classList.contains('menu')
        ) {
          const submenuIntervalo = linkElement
            .closest(`ul#${tipo}`)
            .querySelector(`ul.${tipo}${intervalo}.intervalo`)
          if (submenuIntervalo) {
  ...
  cociente = (valor) => {
    ...
    let valorDenominador =
      denominadores[valor.slice(-2)] || getRandomNumber(1, 10)
    ...
    if (valor.charAt(0) === 'm') {
      ...
      valor.charAt(0) === 't' &&
      operador.classList.contains('fa-minus')
    ) {
      ...
      valor.charAt(0) === 't' &&
      operador.classList.contains('fa-divide')
    ) {
      valorNumerador = valorDenominador * getRandomNumber(1, 10)
    }
  ...
  total = () => {
    ...
    if (valor.charAt(0) === 's') {
      ...
      valor.charAt(0) === 't' &&
      operador.classList.contains('fa-plus')
    ) {
      result = num + den
      contagemOperacoes.faPlus++
    } else if (
      ...
      valor.charAt(0) === 't' &&
      operador.classList.contains('fa-times')
    ) {
      result = num * den
      contagemOperacoes.faTimes++
    } else if (
      ...
      if (response.status === 200) {
        ...
        if (resposta.status === 201) {
          ...
          try {
            ...
            const respostaOperacoes = await fetch(
              '/round/resultado-operacoes',
              {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
