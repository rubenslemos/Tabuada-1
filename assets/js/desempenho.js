document.addEventListener('DOMContentLoaded', function () {
  ...
  function preencherResultadosNoHTML(user) {
    ...
    const containerSection = containerResultado.querySelector(
      '.containerResultados'
    )
    const resultadosSection =
      containerSection.querySelector('.resultados-lista')
    ...
    user.rounds.forEach((round, index) => {
      ...
      rodadaDiv
        .querySelector('.btnContagem')
        .addEventListener('click', function (event) {
          abrirModal(
            `Acertos (%): ${aproveitamento}`,
            `Erros (%): ${erradas}`,
            event,
            index
          )
          ...
          try {
            ...
            if (response.status === 200) {
              ...
              if (user.tipo === 'Professor') {
                ...
                const alunosDaMesmaTurma = users.filter(
                  (u) => u.tipo === 'Aluno' && u.turma === user.turma
                )
                optionsHTML = `
              ...
              const divResultadoAntigo =
                containerResultados.querySelector('.resultadoDiv')
              ...
              selectAlunos.addEventListener('change', function () {
                ...
                if (selectedUserId === user._id) {
                  ...
                  selectedUser = alunosDaMesmaTurma.find(
                    (aluno) => aluno._id === selectedUserId
                  )
                } else if (user.tipo === 'Coordenador') {
              ...
              console.error(
                'Erro ao obter dados dos alunos:',
                response.status,
                response.statusText
              )
